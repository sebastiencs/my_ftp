...
**
** Started on  Mon Mar 16 07:48:04 2015 chapui_s
** Last update Sat Mar 21 23:50:22 2015 chapui_s
*/
...
static int		get_local_ip(t_server *server)
{
  ...
  if (getsockname(fd, (struct sockaddr*)&name, &size) == -1)
    return (-1);
  server->local_ip = strdup(inet_ntop(AF_INET, &name.sin_addr, buffer, 1024));
  close(fd);
  return (0);
}
